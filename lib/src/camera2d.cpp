...
madlib__record__Record_t *madraylib__camera2d__fromRaylib(Camera2D *camera) {
  ...
  fields[0]->name = "offset";
  fields[0]->value = madraylib__math__vector2FromRaylib(&camera->offset);

  fields[1] = (madlib__record__Field_t *)GC_MALLOC_ATOMIC(sizeof(madlib__record__Field_t));
  fields[1]->name = "rotation";
  fields[1]->value = boxDouble((double)camera->rotation);

  fields[2] = (madlib__record__Field_t *)GC_MALLOC(sizeof(madlib__record__Field_t));
  fields[2]->name = "target";
  fields[2]->value = madraylib__math__vector2FromRaylib(&camera->target);

  fields[3] = (madlib__record__Field_t *)GC_MALLOC(sizeof(madlib__record__Field_t));
  fields[3]->name = "zoom";
  fields[3]->value = boxDouble((double)camera->zoom);
