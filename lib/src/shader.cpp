...
void madraylib__shader__beginShaderMode(Shader *shader) { BeginShaderMode(*shader); }
...
int32_t madraylib__shader__getShaderLocation(Shader *shader, char *uniformName) {
  return GetShaderLocation(*shader, uniformName);
}

void madraylib__shader__setShaderLocation(Shader *s, int32_t locationIndex, void *value, int32_t uniformType) {
  void *transformedValue = value;
  if (uniformType == SHADER_UNIFORM_FLOAT) {
    auto _value = (float) unboxDouble((double *)value);
    ...
    float *floats = (float*) GC_MALLOC(sizeof(float) * 2);
    ...
    madlib__record__Record_t *_value = (madlib__record__Record_t *)value;
    float *floats = (float*) GC_MALLOC(sizeof(float) * 3);
    floats[0] = (float)unboxDouble((double *)_value->fields[0]->value);
    floats[1] = (float)unboxDouble((double *)_value->fields[1]->value);
    ...
    madlib__record__Record_t *_value = (madlib__record__Record_t *)value;
    float *floats = (float*) GC_MALLOC(sizeof(float) * 4);
    floats[0] = (float)unboxDouble((double *)_value->fields[0]->value);
    floats[1] = (float)unboxDouble((double *)_value->fields[1]->value);
    floats[2] = (float)unboxDouble((double *)_value->fields[2]->value);
    floats[3] = (float)unboxDouble((double *)_value->fields[3]->value);
    transformedValue = floats;
  }
